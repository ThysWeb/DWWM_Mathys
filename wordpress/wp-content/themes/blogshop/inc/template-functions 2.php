...
if (!function_exists('blogshop_footer_credit')) {
	function blogshop_footer_credit(){
		return '<strong>'.__(' Theme:&nbsp;', 'blogshop').'</strong><a href="'.esc_url('https://theimran.com/themes/wordpress-theme/blogshop-wordpress-theme-for-any-websites/').'"><span> '.__( 'BlogShop', 'blogshop' ).'</span></a>';
	}
}
/**
	...
	function blogshop_bredcrumbs(){
		...
		<nav>
            <div class="breadcrumb">
                <?php
                if(function_exists('bcn_display') && !is_author( )) :
                    bcn_display();
                endif;
                ?>
            </div>
...
if (!function_exists('blogshop_banner_title_control')) {
	function blogshop_banner_title_control(){
		...
		elseif (true === blogshop_is_woocommerce_exists()) :
			?>
...
if (!function_exists('blogshop_default_page_banner')) {
	function blogshop_default_page_banner(){
		if (!is_page_template('frontpage.php')) :
			?>
			 <div class="page-banner-area" style="background-image: url(<?php echo esc_url( get_header_image());?>);">
			    <div class="container">
			        <div class="row">
